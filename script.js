...
function init() {
    ...
    nextBtn.addEventListener('click', goToSkillSelection);
    skillNextBtn.addEventListener('click', goToTimeSelection);
    timeNextBtn.addEventListener('click', generateRecommendations);
    tags.forEach(tag => tag.addEventListener('click', toggleTag));
    timeSlots.forEach(slot => slot.addEventListener('click', toggleTimeSlot));
    searchBtn.addEventListener('click', searchClubs);
    clubType.addEventListener('change', filterClubs);
...
function filterClubs() {
    ...
    if (type !== 'all') {
        filteredClubs = clubs.filter(club => club.type === type);
    }
